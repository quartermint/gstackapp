...
The ancestry check for Mac Mini copies runs via SSH: `ssh mac-mini-host 'cd <path> && git merge-base --is-ancestor <hash1> <hash2>'`
...
Thin MCP server process that calls the Mission Control API (`http://100.x.x.x:3000`) as a client. Does not duplicate scanning logic. Translates API responses into MCP tool results.
