...
describe('GET /api/pipelines', () => {
  ...
  it('returns pipeline list with joined PR/repo/stage data after seeding', async () => {
    ...
    expect(run.pr.number).toBe(42)
    expect(run.pr.title).toBe('Add login feature')
    expect(run.pr.authorLogin).toBe('devuser')
    expect(run.pr.baseBranch).toBe('main')
    expect(run.pr.state).toBe('open')
