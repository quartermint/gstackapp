...
- [ ] **Phase 11: Data Foundation** - Session/budget schema, Drizzle migration, Zod types, model tier config, infra scripts update
...
**Success Criteria** (what must be TRUE):
  1. MC polls LM Studio at `http://100.123.8.125:1234/v1/models` on a timer, and `GET /api/models` returns three-state health (unavailable/loading/ready) for the Qwen3-Coder-30B model, with LM Studio status visible in the existing health panel
...
| 11. Data Foundation | 2/3 | In Progress|  | - |
