...
human_verification:
  ...
  - test: "Tweet content fetch via Crawl4AI"
    expected: "POST /api/captures/import/tweets resolves Capacities tweet URLs to full text content via Crawl4AI at 100.x.x.x:11235"
...
| 5 | Tweet URLs resolve to full text content via Crawl4AI | VERIFIED | `tweet-fetcher.ts` POSTs to Crawl4AI at `100.x.x.x:11235` with OG scraper fallback; `POST /api/captures/import/tweets` endpoint wired; 14 tests pass |
...
| `packages/api/src/services/tweet-fetcher.ts` | Crawl4AI HTTP API | POST to `http://100.x.x.x:11235/crawl` | VERIFIED | `fetchTweetContent` POSTs to `${crawl4aiUrl}/crawl` with 30s timeout |
...
**Why human:** Requires Mac Mini Crawl4AI service running at `100.x.x.x:11235`; cannot stub from automated check
