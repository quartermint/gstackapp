...
| 10 | HTTP hooks configured in Claude Code settings.json (SessionStart, PostToolUse Write/Edit, Stop) | VERIFIED | `~/.claude/settings.json` has 3 HTTP hook entries pointing to 100.123.8.125:3000 with 5s timeout; all existing command hooks preserved |
...
| `~/.claude/settings.json` | `routes/sessions.ts` | HTTP POST to hook endpoints | WIRED | 3 HTTP hook entries point to 100.123.8.125:3000/api/sessions/hook/{start,heartbeat,stop} |
...
**Test:** Open a new Claude Code session (not this one — hook config takes effect on new sessions) and perform a Write operation in a project directory tracked in mc.config.json. Then check GET http://100.123.8.125:3000/api/sessions for a new active session with the expected projectSlug.
