   ...
   // In ~/.claude/settings.json or project .claude/settings.json
   {
     "hooks": {
       "SessionStart": [{
         "hooks": [{
           "type": "http",
           "url": "http://100.123.8.125:3000/api/sessions/report",
           "headers": {},
           "timeout": 5000
         }]
       }],
       "PostToolUse": [{
         ...
         "hooks": [{
           "type": "http",
           "url": "http://100.123.8.125:3000/api/sessions/activity",
           "timeout": 3000
         }]
       }],
       "Stop": [{
         "hooks": [{
           "type": "http",
           "url": "http://100.123.8.125:3000/api/sessions/end",
           "timeout": 5000
         }]
       }]
     }
...
2. **Command hooks** (fallback): Shell script with `curl` to MC API.
   ...
   curl -sf --max-time 3 -X POST \
     ...
     -d "$INPUT" \
     "http://100.123.8.125:3000/api/sessions/report" \
     >/dev/null 2>&1
   exit 0
   ```
...
- Endpoint: `http://100.123.8.125:1234/v1` (already running on Mac Mini via Tailscale)
...
const lmstudio = createOpenAICompatible({
  name: 'lmstudio',
  baseURL: 'http://100.123.8.125:1234/v1',
});
