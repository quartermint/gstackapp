...
| 5 | `mc init` configures API URL with Mac Mini Tailscale default; CLI ships as `packages/cli` following MCP tsup bundle pattern | VERIFIED | `init.ts`: `getDefaultApiUrl()` = 100.x.x.x:3000, `checkHealth()` + `saveConfig()`; `tsup.config.ts`: noExternal, shebang banner, ESM format |
...
| `packages/cli/src/config.ts` | ~/.mc/config.json read/write, Tailscale default URL | VERIFIED | MC_DIR, CONFIG_PATH, DEFAULT_API_URL = http://100.x.x.x:3000, loadConfig/saveConfig |
...
| CLI-09 | 22-01, 22-03, 22-04 | `mc init` configures API URL with Tailscale default | SATISFIED | DEFAULT_API_URL=100.x.x.x:3000, guided setup with checkHealth |
